...
export function SimpleProjectView() {
  const { state, task, output, error, loading, start, plan, implement, review, submit, finish, retry, stop } =
    useProjectStore()
    ...
    <div className="min-h-screen bg-base-100">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        >
          ...
          {task?.title && state !== 'none' && <p className="text-sm opacity-70 mt-1 truncate">{task.title}</p>}
        </div>
        ...
        {outputTail.length > 0 && (
          <div className="bg-base-200 rounded-lg p-3 max-h-40 overflow-y-auto">
            <pre className="text-xs text-base-content/60 font-mono whitespace-pre-wrap">{outputTail.join('\n')}</pre>
          </div>
        )}
        ...
        <section aria-label="Actions" className="space-y-4">
          ...
          {state === 'none' && (
            <div className="space-y-3">
              ...
              <textarea
                ...
                placeholder={
                  'Paste a GitHub issue URL, file path, or describe the task...\n\nExamples:\n  https://github.com/org/repo/issues/42\n  file:task.md\n  Add error handling to the login flow'
                }
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={handleStart} disabled={!taskSource.trim() || loading} className="btn btn-primary">
                ...
                <div className="border border-base-300 rounded-lg overflow-hidden">
                  {filesLoading ? (
                    ...
                    <div className="text-sm text-base-content/50 py-4 text-center">
                      No task files found (.md, .txt, .yaml)
                    </div>
                  ) : (
          ...
          {state === 'loaded' && (
            <div className="text-center">
              <button onClick={() => void plan()} disabled={loading} className="btn btn-primary btn-lg">
          ...
          {state === 'planned' && (
            <div className="space-y-4">
              {specs.length > 0 && (
                <div className="border border-base-300 rounded-lg overflow-hidden">
                  ...
                  >
                    <span>View Plan</span>
                    <svg
                      className={`w-4 h-4 transition-transform ${showSpec ? 'rotate-180' : ''}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
              ...
              <div className="text-center">
                <button onClick={() => void implement()} disabled={loading} className="btn btn-primary btn-lg">
          ...
          {isActive && (
            <div className="text-center">
              <button onClick={() => void stop()} className="btn btn-ghost btn-sm">
                Stop
              </button>
            </div>
          )}
          ...
          {state === 'submitted' && (
            <div className="text-center space-y-3">
              ...
              {(() => {
                const prLine = [...output]
                  .reverse()
                  .find((l: string) => l.includes('http') && (l.includes('/pull/') || l.includes('/merge_requests/')))
              ...
              <div>
                <button onClick={handleFinish} disabled={loading} className="btn btn-primary btn-lg">
          ...
          {state === 'failed' && (
            <div className="text-center space-y-2">
              {error && <p className="text-sm text-error bg-error/10 rounded-lg px-4 py-2">{error}</p>}
              <button onClick={() => void retry()} disabled={loading} className="btn btn-warning">
          ...
          {state === 'paused' && (
            <div className="text-center">
              <button onClick={() => void retry()} disabled={loading} className="btn btn-primary">
                Resume
              </button>
            </div>
          )}
