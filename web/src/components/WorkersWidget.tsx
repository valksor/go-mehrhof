    ...
    <div>
      ...
      {workerStats && (
        <div className="grid grid-cols-2 gap-2 mb-4">
          ...
          <div className="stat bg-base-300 rounded-lg p-3">
            ...
            <div className="stat-value text-lg">{workerStats.in_progress_jobs}</div>
            <div className="stat-desc text-xs">{workerStats.queued_jobs} queued</div>
          </div>
        </div>
      )}
      ...
      {workers.length === 0 ? (
        ...
        <div className="space-y-2 max-h-[300px] overflow-auto">
          {workers.map((worker) => (
            <div key={worker.id} className="p-3 rounded-lg bg-base-300 border border-transparent">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-sm text-base-content">
                    {worker.agent_name}
                    {worker.is_default && <span className="ml-2 badge badge-xs badge-primary">default</span>}
                  </div>
    ...
    <section className="card bg-base-200">
      <div className="card-body">
        <h2 className="card-title text-base-content flex items-center gap-2">
          ...
          Workers
          {workerStats && <span className="badge badge-sm badge-ghost">{workerStats.total_workers}</span>}
        </h2>
