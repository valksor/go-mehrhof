    ...
    <div>
      {hasStatusCounts && (
        <div className="flex gap-2 text-xs flex-wrap mb-2 px-1">
          {gitStatus?.branch && <span className="badge badge-sm badge-outline font-mono">{gitStatus.branch}</span>}
          {statusCounts.added > 0 && (
            <span className="badge badge-sm badge-success badge-outline">{statusCounts.added} added</span>
          )}
          {statusCounts.modified > 0 && (
            <span className="badge badge-sm badge-warning badge-outline">{statusCounts.modified} modified</span>
          )}
          {statusCounts.deleted > 0 && (
            <span className="badge badge-sm badge-error badge-outline">{statusCounts.deleted} deleted</span>
          )}
        </div>
      )}
      {!fileChanges || fileChanges.length === 0 ? (
        <EmptyState
          ...
          description="File changes will appear here during implementation"
          icon="📄"
        />
      ) : (
        <div className="space-y-1 max-h-[250px] overflow-auto">
          {(fileChanges as FileChange[]).map((file, index) => {
            ...
            return (
              ...
              >
                <span
                  className={`w-6 h-6 rounded flex items-center justify-center font-mono text-sm ${config.bg} ${config.color}`}
                >
                  {config.icon}
                </span>
