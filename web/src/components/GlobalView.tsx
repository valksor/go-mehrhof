    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 bg-base-100">
      ...
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 sm:mb-8">
        <div className="flex items-center gap-3">
          <div
            className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-primary flex items-center justify-center shadow-lg"
            aria-hidden="true"
          >
            <svg
              className="w-4 h-4 sm:w-5 sm:h-5 text-primary-content"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          ...
          {!connected && !connecting && reconnectAttempt === 0 && (
            <button onClick={() => connect()} className="btn btn-warning btn-sm" aria-label="Reconnect">
          ...
          {!connected && !connecting && reconnectAttempt > 0 && (
            <span
              className="text-sm text-warning flex items-center gap-2 bg-warning/10 px-3 py-1.5 rounded-lg"
              role="status"
              aria-live="polite"
            >
          ...
          {error && (
            <span
              role="alert"
              title={error}
              className="text-xs sm:text-sm text-error bg-error/10 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg border border-error/20 max-w-[200px] sm:max-w-none whitespace-normal break-words"
            >
              {error}
            </span>
          )}
          ...
          {!isSimple && hasActiveProjects && connected && (
            <div className="dropdown dropdown-end">
              ...
              <ul
                tabIndex={0}
                role="menu"
                className="dropdown-content z-10 menu p-2 shadow-lg bg-base-200 rounded-box w-56"
              >
                ...
                {['plan', 'implement', 'review', 'submit', 'stop', 'abort', 'reset', 'pause', 'resume'].map(
                  (action) => (
                    <li key={action}>
                      <button onClick={() => handleBatchAction(action)} disabled={batchRunning} className="capitalize">
                        {action}
                      </button>
                    </li>
                  ),
      ...
      <section className="card bg-base-200 max-w-2xl mx-auto mt-4">
        <div className="card-body p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            ...
            <button onClick={() => setShowFolderPicker(true)} className="btn btn-primary btn-sm w-full sm:w-auto">
              <svg aria-hidden="true" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {projects.length === 0 ? (
            <div className="text-center py-8 sm:py-12">
              <div
                aria-hidden="true"
                className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-base-300 flex items-center justify-center mx-auto mb-4"
              >
                <svg
                  className="w-6 h-6 sm:w-8 sm:h-8 text-base-content/50"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-2">
                <button onClick={() => setShowFolderPicker(true)} className="btn btn-primary btn-sm">
                  <svg aria-hidden="true" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                  </svg>
                  Add Project
                ...
                {docsData?.url && (
                  <a href={docsData.url} target="_blank" rel="noopener noreferrer" className="btn btn-ghost btn-sm">
                    <svg aria-hidden="true" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
            ...
            <ul aria-label="Projects" className="space-y-2 max-h-[300px] sm:max-h-[400px] overflow-auto">
              {filteredProjects.length === 0 && searchQuery && (
                <li className="text-center py-4 text-base-content/50 text-sm">No projects matching "{searchQuery}"</li>
              )}
              {filteredProjects.map((p) => {
                ...
                return (
                  <li
                    key={p.id}
                    ref={selectedProject?.id === p.id ? selectedRef : undefined}
                  ...
                  >
                    ...
                    >
                            ...
                            className={`badge badge-sm sm:badge-md ${
                              p.state === 'none'
                                ...
                                : p.state === 'implemented'
                                  ...
                                  : p.state === 'failed'
                                    ? 'badge-error'
                                    : p.state === 'planning' || p.state === 'implementing' || p.state === 'reviewing'
                                      ? 'badge-warning'
                                      : 'badge-primary'
                            }`}
                          >
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {task?.source && (
                          <span className="text-xs text-primary/60 font-mono truncate flex-shrink-0">
                            {task.source}
                          </span>
                        )}
                        {task?.queue_count != null && task.queue_count > 0 && (
                          <span className="badge badge-xs badge-outline flex-shrink-0">+{task.queue_count} queued</span>
                        )}
                      </div>
                    </button>
                    <button
      ...
      <Suspense fallback={null}>
        ...
        {showDiagnose && <DiagnosePanel isOpen={showDiagnose} onClose={() => setShowDiagnose(false)} />}
        {showRecordings && <RecordingsPanel isOpen={showRecordings} onClose={() => setShowRecordings(false)} />}
        {showBackup && <BackupPanel isOpen={showBackup} onClose={() => setShowBackup(false)} />}
        {showActivity && <ActivityPanel isOpen={showActivity} onClose={() => setShowActivity(false)} />}
        {showSecurity && <SecurityPanel isOpen={showSecurity} onClose={() => setShowSecurity(false)} />}
        {showCatalog && <CatalogPanel isOpen={showCatalog} onClose={() => setShowCatalog(false)} />}
        {showReport && <ReportPanel isOpen={showReport} onClose={() => setShowReport(false)} />}
        {showExport && <ExportPanel isOpen={showExport} onClose={() => setShowExport(false)} />}
        {showTaskGroups && <TaskGroupPanel onClose={() => setShowTaskGroups(false)} />}
      </Suspense>

      {/* First-run onboarding (only shows once) */}
      {projects.length === 0 && connected && <Onboarding onAddProject={() => setShowFolderPicker(true)} />}
    </div>
  )
}
