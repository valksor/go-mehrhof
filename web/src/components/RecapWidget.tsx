    ...
    <section className="card bg-base-200">
      <div className="card-body gap-3">
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          ...
          {recap.last_activity && (
            <span className="text-base-content/60">{recap.last_activity}</span>
          )}
        </div>
          ...
          <div className="flex flex-wrap gap-1">
            {['plan', 'implement', 'simplify', 'optimize', 'review'].map(phase =>
              recap.phase_metrics?.[phase] ? (
                <span key={phase} className="badge badge-xs badge-ghost">{phase}</span>
              ) : null
            )}
          </div>
        )}
            ...
            <div className="max-h-24 overflow-auto space-y-0.5">
              ...
              {recap.files_changed.length > 8 && (
                <div className="text-base-content/50">
                  +{recap.files_changed.length - 8} more
                </div>
              )}
            </div>
          </div>
        ...
        {recap.last_checkpoint && (
          <div className="text-xs text-base-content/60">
            ...
            {recap.last_checkpoint.message && (
              <span> &mdash; {recap.last_checkpoint.message}</span>
            )}
          </div>
        )}
        ...
        {/* Error */}
        {recap.last_error && (
          <div className="text-xs text-error">{recap.last_error}</div>
        )}
