...
export function TaskWidget({ embedded = false }: TaskWidgetProps) {
  const { task, state, start, quickStart, queueTask, loading, error, connected, connecting, undo, reset, phaseError, needsRecovery, ciFixStatus } = useProjectStore()
    ...
    <>
      ...
      {needsRecovery && (
        <div className="alert alert-warning text-sm mt-3">
          ...
          <span>Task was interrupted during <strong>{needsRecovery}</strong>. Run <code className="bg-base-300 px-1 rounded">kvelmo retry</code> to resume.</span>
        </div>
      )}
