...
describe('ChangelogPanel', () => {
  ...
  it('does not render when closed', () => {
    const { queryByRole } = render(<ChangelogPanel isOpen={false} onClose={vi.fn()} />)
    expect(queryByRole('dialog')).not.toBeInTheDocument()
  })

  it('renders modal with title when open', () => {
    const { getByText } = render(<ChangelogPanel isOpen={true} onClose={vi.fn()} />)
    expect(getByText('Changelog')).toBeInTheDocument()
  })

  it('shows placeholder text initially', () => {
    const { getByText } = render(<ChangelogPanel isOpen={true} onClose={vi.fn()} />)
    expect(getByText('Enter source and target refs, then click Generate.')).toBeInTheDocument()
  })
  ...
  it('generate button is disabled when disconnected', () => {
    mockConnected = false
    const { getByText, getByLabelText } = render(<ChangelogPanel isOpen={true} onClose={vi.fn()} />)
  ...
  it('shows content after generation', async () => {
    ...
    const { getByText, getByLabelText, findByText } = render(<ChangelogPanel isOpen={true} onClose={vi.fn()} />)
  ...
  it('shows loading spinner during generation', () => {
    ...
    const { getByText, getByLabelText, container } = render(<ChangelogPanel isOpen={true} onClose={vi.fn()} />)
  ...
  it('shows generic error for non-Error failure', async () => {
    ...
    expect(await findByText('Failed to generate changelog')).toBeInTheDocument()
  })
  ...
  it('shows copy button when content exists', async () => {
    ...
    const { getByText, getByLabelText, findByLabelText } = render(<ChangelogPanel isOpen={true} onClose={vi.fn()} />)
  ...
  it('full descriptions checkbox toggles', async () => {
    ...
    const checkbox = getByText('Full descriptions').closest('label')!.querySelector('input[type="checkbox"]')!
  ...
  it('calls onClose when close button clicked', () => {
    const onClose = vi.fn()
    const { getByLabelText } = render(<ChangelogPanel isOpen={true} onClose={onClose} />)
