...
export const inspectCommands: ChatCommand[] = [
  ...
  },
  {
    ...
    execute: async () => {
      ...
      const result = await client.call<{ checkpoints: Array<{ sha: string; message: string; timestamp: string }> }>(
        'checkpoints',
        {},
      )
  ...
  },
  {
    ...
    execute: async (args) => {
      ...
      const client = worktreeClient()
      if (!client) return 'Not connected.'
      const result = await client.call<{ tasks: Array<{ id: string; title: string; state: string }> }>('task.search', {
        query,
      })
  ...
  },
  {
    ...
    execute: async () => {
      const client = worktreeClient()
      if (!client) return 'Not connected.'
      const result = await client.call<{ tasks: Array<{ id: string; title: string; state: string }> }>(
        'task.history',
        {},
      )
      const tasks = result.tasks || []
  ...
  },
  {
    ...
    execute: async () => {
      const client = worktreeClient()
      if (!client) return 'Not connected.'
      const result = await client.call<{ events: Array<{ type: string; timestamp: string; message?: string }> }>(
        'eventlog.query',
        { limit: 20 },
      )
